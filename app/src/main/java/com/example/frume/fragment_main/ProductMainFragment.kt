...
class ProductMainFragment(val combinationFragment: CombinationFragment) : Fragment() {
    ...
    private fun replaceFragmentByArguments() {
        when (productInfoType) {
            ...
            ProductInfoType.USER_PRODUCT_INFO_TYPE -> replaceFragment(
                ProductSubFragment.USER_PRODUCT_INFO_FRAGMENT,
                true,
                true,
                null
            )
            // 상품 설명
            ProductInfoType.USER_PRODUCT_INFO_DESCRIPTION_TYPE -> replaceFragment(
                ProductSubFragment.USER_PRODUCT_INFO_DESCRIPTION_FRAGMENT,
                true,
                true,
                null
            )
            // 상품 상세 설명
            ProductInfoType.USER_PRODUCT_INFO_DETAIL_TYPE -> replaceFragment(
                ProductSubFragment.USER_PRODUCT_INFO_DETAIL_FRAGMENT,
                true,
                true,
                null
            )
            // 상품 리뷰
            ProductInfoType.USER_PRODUCT_INFO_REVIEW_TYPE -> replaceFragment(
                ProductSubFragment.USER_PRODUCT_INFO_REVIEW_FRAGMENT,
                true,
                true,
                null
            )
            // 상품 리뷰 쓰기
            ProductInfoType.USER_PRODUCT_INFO_WRITE_TYPE -> replaceFragment(
                ProductSubFragment.USER_PRODUCT_INFO_WRITE_REVIEW_FRAGMENT,
                true,
                true,
                null
            )
            // 상품 구매 바텀시트
            ProductInfoType.USER_PRODUCT_INFO_DIALOG_TYPE -> replaceFragment(
                ProductSubFragment.USER_PRODUCT_INFO_DIALOG_FRAGMENT,
                true,
                true,
                null
            )
            // 상품 리스트
            ProductInfoType.USER_PRODUCT_SHOW_LIST_TYPE -> replaceFragment(
                ProductSubFragment.USER_PRODUCT_SHOW_LIST_FRAGMENT,
                true,
                true,
                null
            )
...
enum class ProductSubFragment(var number: Int, var str: String) {
    ...
    // 카테고리 선택 -> 상품 리스트 뷰 ( 상품 리스트뷰를 보는게 아니라 프로덕트 메인으로 왔으면 안됐습니다 죄송합니다. 25.01.17 hj
